...
open class FolioReaderCenter: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    fileprivate func updateSubviewFrames() {
        pageIndicatorView?.frame = self.frameForPageIndicatorView()
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        return configure(readerPageCell: reuseableCell, atIndexPath: indexPath)
    }
...
extension FolioReaderCenter: FolioReaderChapterListDelegate {
    ...
    func chapterList(_ chapterList: FolioReaderChapterList, didSelectRowAtIndexPath indexPath: IndexPath, withTocReference reference: FRTocReference) {
        ...
        if item < totalPages {
            let indexPath = IndexPath(row: item, section: 0)
            changePageWith(indexPath: indexPath, animated: false, completion: { [weak self] () -> Void in
                self?.updateCurrentPage()
            })
    ...
    // MARK: - Public wrapper methods for external access

    /// Public wrapper to navigate to a chapter without requiring FolioReaderChapterList instance
    public func navigateToChapter(atIndexPath indexPath: IndexPath, withTocReference reference: FRTocReference) {
        let item = findPageByResource(reference)

        if item < totalPages {
            let pageIndexPath = IndexPath(row: item, section: 0)
            changePageWith(indexPath: pageIndexPath, animated: false, completion: { [weak self] () -> Void in
                self?.updateCurrentPage()
            })
            tempReference = reference
        } else {
    ...
    /// Public wrapper to update page after chapter selection
    public func updateAfterChapterSelection() {
        updateCurrentPage()

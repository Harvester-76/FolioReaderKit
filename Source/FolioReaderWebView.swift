...
// MARK: - Custom Extensions for Project
extension FolioReaderWebView {

    /// Highlight all occurrences of a search term and scroll to specific index
    public func highlightAllOccurencesAndMoveToIndex(of searchTerm: String, index: Int, fullString: String) {
        let escapedSearch = searchTerm.replacingOccurrences(of: "'", with: "\\'")
        let escapedFull = fullString.replacingOccurrences(of: "'", with: "\\'")
        let script = "highlightAllOccurrencesAndMoveToIndex('\(escapedSearch)', \(index), '\(escapedFull)')"
        js(script) { _ in }
    }

    /// Highlight a commented text with specific style
    public func highlightCommented(_ comment: String, style: HighlightStyle) {
        let escapedComment = comment.replacingOccurrences(of: "'", with: "\\'")
        let script = "highlightCommented('\(escapedComment)', '\(HighlightStyle.classForStyle(style.rawValue))')"
        js(script) { _ in }
    }

    /// Scroll to a specific highlight by ID
    public func scrollToHighlight(id: String) {
        let script = "scrollToHighlight('\(id)')"
        js(script) { _ in }
    }
}
